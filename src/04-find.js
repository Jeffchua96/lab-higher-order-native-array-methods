...
function findPinkElephantsByTimestreet(songs) {
  return song = songs.find(song => song.title === "Pink Elephants")
}
...
function findFirstSongUnderThreeMinutes(songs) {
  return song = songs.find(song => song.runtimeInSeconds <= "180")
}
/**
...
function findFirstTitleTrack(songs) {
  return song = songs.find(song =>song.title === song.album)
}
